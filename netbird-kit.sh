...
cmd_init() {
  ...
  # Two NetBird stacks on one host will collide on project name, container
  # names, and volume names unless this differs between them.
  local _defproj; _defproj="$(basename "$SCRIPT_DIR" | tr -cd 'a-z0-9-' )"
  [[ -z "$_defproj" ]] && _defproj="netbird"
  ask NB_PROJECT "Compose project name (must be unique per host)" "$_defproj"
  if ! [[ "$NB_PROJECT" =~ ^[a-z0-9][a-z0-9_-]*$ ]]; then
    die "Project name must be lowercase alphanumeric, may contain - or _."
  fi
  if command -v docker >/dev/null 2>&1 \
     && docker compose ls --all --format json 2>/dev/null | grep -q "\"Name\":\"${NB_PROJECT}\""; then
    warn "A compose project named '${NB_PROJECT}' already exists on this host."
    ask_yesno "Continue anyway (it will be reused/overwritten)?" "n" \
      || die "Aborted. Choose a different project name."
  fi
...
# --- Compose project (namespaces containers, volumes, network) ---
NB_PROJECT=${NB_PROJECT}
COMPOSE_PROJECT_NAME=${NB_PROJECT}
...
emit_traefik_service() {
  ...
  traefik:
    ...
    container_name: ${NB_PROJECT}-traefik
...
HEAD
  ...
  netbird-server:
    image: ${NB_SERVER_IMG}
    container_name: ${NB_PROJECT}-server
    restart: unless-stopped
    depends_on:
      postgres:
...
SRV
  ...
  dashboard:
    image: ${NB_DASH_IMG}
    container_name: ${NB_PROJECT}-dashboard
    restart: unless-stopped
...
DASH
  ...
  postgres:
    image: ${NB_PG_IMG}
    container_name: ${NB_PROJECT}-postgres
    restart: unless-stopped
    environment:
...
# Identity provider guidance. The combined netbird-server container always runs
# the embedded Dex IdP — it cannot be disabled or swapped via config.yaml — so
# external IdPs are added at runtime from the Dashboard, not generated here.
print_advice_idp() {
  echo
  info "Identity / SSO"
  echo  "  This deployment uses NetBird's built-in (embedded Dex) identity provider."
  echo  "  It is fully functional on its own: create the first admin at"
  echo  "  ${NB_EXPOSED_ADDRESS}/setup, then add users from the Dashboard."
  echo
  echo  "  To add Keycloak, Authentik, Google, Microsoft Entra, Okta, or any other"
  echo  "  OIDC provider, do it IN THE DASHBOARD — not in config.yaml:"
  echo  "    Settings > Identity Provider > add provider"
  echo  "  This is the supported path. External providers run ALONGSIDE local users,"
  echo  "  can be added without restarting anything, and support optional JWT group"
  echo  "  sync so IdP groups map to NetBird groups automatically."
  echo
  echo  "  Note: the combined netbird-server container always enables the embedded"
  echo  "  IdP; it cannot be replaced via configuration. Adding an external provider"
  echo  "  in the Dashboard is the correct approach, not a workaround."
  echo  "  Docs: https://docs.netbird.io/selfhosted/identity-providers/keycloak"
}
...
cmd_health() {
  ...
  # With ACME staging the cert is intentionally untrusted, so skip verification
  # for the reachability check and say so rather than reporting a false failure.
  local curl_tls_opts=() staging
  staging="$(get_env NB_ACME_STAGING)"
  if [[ "$staging" == "true" ]]; then
    curl_tls_opts+=(-k)
